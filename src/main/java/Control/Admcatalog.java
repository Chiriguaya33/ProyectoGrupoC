...
"WHERE INVENTARIO.NombreProducto LIKE ? ";
        ...
        try (Connection conn = ConexionBD.conectar();
            ...
            // El % se usa para buscar coincidencias parciales

            ResultSet rs = statement.executeQuery();
            if(!rs.next()){
             throw new Exceptions(" Producto no disponible.");
            }else{
                rs = statement.executeQuery();
           while(rs.next()){
                  Catalogo ctg = new Catalogo();
               ctg.setIdcata(rs.getInt("ID_cata"));
                ctg.setNombre(rs.getString("Nombre Producto"));
                ctg.setMarca( rs.getString("Marca"));
                ctg.setDetalles(rs.getString("Descripcion"));
                
              ctg.setPrecio(rs.getFloat("Precio Unitario"));
        ...
        if (imagenBytes!= null) {
            try {
                ...
                e.printStackTrace();
               throw new Exceptions("Imagen no disponible");
            }
        }
